...
NOTE: The Quickstart requires Java 25 and a recent Git (2.46 or newer preferred) to be available on your machine.
...
----
$ java -version
oopenjdk version "25.0.3" 2026-04-21 LTS
...
Java version: 25.0.3, vendor: Eclipse Adoptium, runtime: /Library/Java/JavaVirtualMachines/temurin-25.jdk/Contents/Home
...
Spring module to easily build data access layers using JPA 3.2 (Jakarta Persistence API).
...
There are a number of tests in this project. You can run them with the following command:
[source, shell]
----
$ ./mvnw test
----

All levels of tests are present in this project, including unit tests, integration tests and end-to-end tests.
The end-to-end tests are implemented using Selenide.
Read the `src/test/java/selenidePageObjects` package for more information (Readme).